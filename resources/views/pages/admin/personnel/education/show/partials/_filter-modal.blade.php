<x-ui.modal show="filterModalOpen" maxWidth="lg">
    ...
    {{-- Form filter (1 baris per input berurutan vertikal) --}}
    <div id="staff-filter-form" class="flex-1 overflow-y-auto p-6 space-y-5">

        {{-- 1. Status Kepegawaian (Employment Status) --}}
        <div>
            <label class="block text-sm font-bold text-foreground mb-2">Status Kepegawaian</label>
            <select name="filter_employment_status" class="w-full bg-white border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary transition-all">
                <option value="">Semua Status Kepegawaian</option>
                @foreach ($employmentOptions ?? [] as $id => $name)
                <option value="{{ $id }}" @selected(isset($filterEmploymentStatus) && $filterEmploymentStatus==$id)>{{ $name }}</option>
                @endforeach
            </select>
        </div>

        {{-- 2. Jenis Pegawai (Personnel Type) --}}
        <div>
