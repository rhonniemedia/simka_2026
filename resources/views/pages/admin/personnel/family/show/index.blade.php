...
@section('title', 'Detail Keluarga: ' . $staff->name)
@section('page_title', 'Data Keluarga Pegawai')
@section('page_subtitle', 'Detail anggota keluarga untuk ' . $staff->name)
...
<div class="px-5 py-8 md:p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-secondary">Kelola data pasangan dan anak untuk <span class="font-bold text-foreground">{{ $staff->name }}</span>.</p>
        </div>
        <div class="flex items-center gap-2 sm:gap-3 w-full md:w-auto mt-2 md:mt-0">
            {{-- Tombol Tambah dengan HTMX --}}
            <button type="button"
                hx-get="{{ route('admin.personnel.family.create', $staff->id) }}"
    ...
    {{-- 3. TABEL DATA KELUARGA --}}
    <div class="bg-white rounded-2xl border border-border p-5">
        <div class="mb-5">
            <h2 class="text-lg font-bold text-foreground">Daftar Anggota Keluarga</h2>
        </div>

        @include('pages.admin.personnel.family.show.partials._table', compact('staff', 'families'))
    </div>
</div>
@endsection