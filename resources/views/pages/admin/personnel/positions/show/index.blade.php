...
@section('title', 'Riwayat Jabatan ASN: ' . $staff->name)
@section('page_title', 'Riwayat Jabatan ASN')
@section('page_subtitle', 'Detail riwayat jabatan untuk ' . $staff->name)
@section('content')
<div class="px-5 py-8 md:p-8">
    {{-- 1. PAGE HEADER --}}
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-foreground mb-1">Riwayat Jabatan ASN</h1>
            <p class="text-sm text-secondary">Kelola riwayat jabatan fungsional/pelaksana untuk <span class="font-bold text-foreground">{{ $staff->name }}</span>.</p>
        </div>
        ...
        <div class="flex items-center gap-2 sm:gap-3 w-full md:w-auto mt-2 md:mt-0">
            {{-- Tombol Tambah --}}
            <button type="button"
                hx-get="{{ route('admin.personnel.positions.create', $staff->id) }}"
    ...
    <div class="bg-gradient-to-br from-primary/5 via-slate-50/50 to-white rounded-2xl border border-primary/15 p-4 sm:p-5 mb-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-5">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 bg-white/60 backdrop-blur-sm border border-border rounded-xl p-3.5 sm:px-6 sm:py-3 w-full md:w-auto">
                ...
                {{-- Jabatan ASN Aktif --}}
                <div class="flex items-center gap-3.5 min-w-0">
                    <div class="flex items-center justify-center size-8 sm:size-9 rounded-lg bg-emerald-100 text-emerald-600 shrink-0">
                        <i data-lucide="briefcase-business" class="size-4 sm:size-4.5"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-[10px] sm:text-[11px] font-bold text-secondary uppercase tracking-widest mb-0.5">Jabatan ASN Aktif</p>
                        <p class="truncate text-[13px] sm:text-sm font-semibold text-foreground/90">
                            {{ $activePosition?->asnPosition?->name ?? 'Belum ada' }}
                        </p>
                    </div>
                </div>
    ...
    {{-- 3. TABEL RIWAYAT JABATAN ASN --}}
    <div class="bg-white rounded-2xl border border-border p-5">
        <div class="mb-5">
            <h2 class="text-lg font-bold text-foreground">Daftar Riwayat Jabatan ASN</h2>
        </div>
        @include('pages.admin.personnel.positions.show.partials._table', compact('staff', 'histories'))
    </div>
</div>
