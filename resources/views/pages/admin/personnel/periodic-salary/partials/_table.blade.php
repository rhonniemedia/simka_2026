{{-- File: resources/views/pages/admin/payroll/periodic-salary/partials/_table.blade.php --}}
<div id="periodic-salary-container"
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="border-b border-border">
                    <th class="w-[35%] px-4 py-3 text-sm font-bold text-secondary tracking-wider">
                        Data Pegawai
                    ...
                    <th class="w-[25%] px-4 py-3 text-sm font-bold text-secondary tracking-wider">
                        Gaji & TMT
                        <div class="text-[11px] font-normal normal-case">Status | Terhitung Mulai Tanggal</div>
                    </th>
                    <th class="w-[15%] px-4 py-3 text-sm font-bold text-secondary tracking-wider">
                        Aksi
            ...
            <tbody>
                ...
                $tanggalSk = $r->decree_date ? \Carbon\Carbon::parse($r->decree_date)->translatedFormat('d M Y') : '-';
                $tmt = $r->effective_date ? \Carbon\Carbon::parse($r->effective_date)->translatedFormat('d M Y') : '-';
                ...
                <tr id="row-salary-{{ $r->id }}" class="border-b border-border hover:bg-muted/50 transition-colors">
                        ...
                        <div class="flex items-center gap-3 group transition-all">
                            ...
                            <div>
                                <div class="font-semibold text-foreground text-sm uppercase group-hover:text-primary transition-colors whitespace-nowrap">
                                    {{ $staff?->name ?? 'Data Pegawai Tidak Ditemukan' }}
                                </div>
                    ...
                    <td class="px-5 py-4 min-w-[160px]">
                        <div class="text-sm font-medium text-foreground whitespace-nowrap">{{ $r->decree_number ?? 'Belum ada Nomor SK' }}</div>
                        <div class="flex items-center gap-1 text-xs text-secondary whitespace-nowrap mt-1">
                            ...
                            <span class="leading-none mt-[1px]">{{ $tanggalSk }}</span>
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-4 min-w-[160px]">
                        ...
                        <div class="flex items-center gap-1 text-xs text-secondary whitespace-nowrap mt-1.5">
                            <span class="font-medium">TMT:</span> {{ $tmt }}
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-4 min-w-[120px]">
                        <div x-data="{
                            ...
                            <div x-ref="menu" x-show="open" x-cloak
                                ...
                                {{-- Ganti route di bawah ini sesuai dengan nama route asli Anda nantinya --}}
                                <button type="button" @click="open = false" hx-get="#" hx-target="#modal-container" hx-swap="outerHTML" class="flex items-center gap-2 mx-2 px-3 py-2 rounded-lg text-sm text-foreground hover:bg-muted transition-colors cursor-pointer text-left">
                                    <i data-lucide="file-text" class="size-4 text-secondary pointer-events-none"></i> Lihat Detail SK
                                </button>
                                ...
                                <button type="button" hx-delete="#" hx-target="#periodic-salary-container" hx-select="#periodic-salary-container" hx-swap="outerHTML" hx-confirm="Yakin ingin menghapus data riwayat ini? Tindakan ini tidak dapat dibatalkan." class="flex items-center gap-2 mx-2 px-3 py-2 rounded-lg text-sm text-error hover:bg-error/10 transition-colors cursor-pointer text-left">
    ...
    <div class="lg:hidden divide-y divide-border border-y border-border bg-white -mx-5 mb-5 mt-2">
        ...
        <div id="card-salary-{{ $r->id }}" class="px-5 py-4 border-border hover:bg-muted/40 active:bg-muted/60 transition-colors">
            <div class="flex items-start gap-3">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <div class="font-semibold text-foreground text-sm uppercase truncate block">
                                {{ $staff?->name ?? 'Data Pegawai Tidak Ditemukan' }}
                            </div>
                    ...
                    <div class="mt-3 border-t border-b border-border divide-y divide-border text-xs">
                        ...
                        <div class="flex items-center justify-between gap-3 py-2.5">
                            <p class="text-secondary flex items-center gap-1.5 shrink-0">
                                <i data-lucide="shield-check" class="size-3 text-slate-400"></i>
                                Status
                            </p>
                        ...
                        <div class="flex items-center justify-between gap-3 py-2.5">
                            ...
                            <div class="text-right min-w-0 flex-1">
                                <p class="font-semibold text-foreground truncate">{{ $r->decree_number ?? '-' }}</p>
                                <p class="text-secondary truncate mt-0.5">Tgl: {{ $tanggalSk }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-3 py-2.5">
                            ...
                            <div class="text-right min-w-0">
                                <p class="text-foreground font-medium truncate">{{ $tmt }}</p>
                            </div>
                        </div>
                    </div>
                </div>
