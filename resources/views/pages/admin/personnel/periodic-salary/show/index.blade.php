{{-- File: resources/views/pages/admin/personnel/periodic-salary/show/index.blade.php --}}
@extends('layouts.main.admin')
@section('title', 'Detail Gaji Berkala: ' . $staff->name)
@section('page_title', 'Riwayat Gaji Berkala Pegawai')
@section('page_subtitle', 'Detail riwayat gaji berkala untuk ' . $staff->name)
@section('content')
<div class="px-5 py-8 md:p-8">
    {{-- 1. PAGE HEADER --}}
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-foreground mb-1">Riwayat Gaji Berkala</h1>
            <p class="text-sm text-secondary">Kelola riwayat gaji berkala untuk <span class="font-bold text-foreground">{{ $staff->name }}</span>.</p>
        </div>
        ...
        <div class="flex items-center gap-2 sm:gap-3 w-full md:w-auto mt-2 md:mt-0">
            {{-- Tombol Tambah --}}
            <button type="button"
                hx-get="{{ route('admin.personnel.periodic-salary.create', $staff->id) }}"
    ...
    <div class="bg-gradient-to-br from-primary/5 via-slate-50/50 to-white rounded-2xl border border-primary/15 p-4 sm:p-5 mb-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-5">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="min-w-0 flex flex-col justify-center pt-0.5">
                    <h2 class="text-base sm:text-[1.3rem] font-bold text-foreground uppercase truncate tracking-tight leading-none">
                        {{ $staff->name }}
                    </h2>
                    <p class="text-xs sm:text-sm text-secondary mt-1.5 leading-none">
                        Profil Pegawai
                    </p>
                </div>
            </div>
    ...
    {{-- 3. TABEL RIWAYAT GAJI BERKALA --}}
    <div class="bg-white rounded-2xl border border-border p-5">
        <div class="mb-5">
            <h2 class="text-lg font-bold text-foreground">Daftar Riwayat Gaji Berkala</h2>
        </div>

        {{-- Memanggil partial _table.blade.php yang ada di folder show/partials --}}
        @include('pages.admin.personnel.periodic-salary.show.partials._table', compact('staff', 'histories'))
    </div>
</div>
@endsection