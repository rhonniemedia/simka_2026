{{-- File: resources/views/pages/admin/personnel/retirements/partials/_filter-modal.blade.php --}}
<x-ui.modal show="filterModalOpen" maxWidth="lg">
    ...
    <div class="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-border shrink-0 bg-gray-50/50">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-foreground text-base sm:text-lg">Filter Data Pensiun</h3>
    ...
    {{-- Form filter --}}
    <div id="retirement-filter-form" class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 sm:space-y-6">
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                    <label class="block text-sm text-foreground mb-2">Status Pensiun</label>
                    <x-ui.select
                        name="filter_retirement_status"
                        :options="$retirementStatusOptions"
                        value="{{ $filterRetirementStatus ?? '' }}"
                        placeholder="Semua Status" />
                </div>
                <div>
                    <label class="block text-sm text-foreground mb-2">Tahun Pensiun (TMT)</label>
                    <x-ui.select
                        name="filter_year"
                        :options="$yearOptions"
                        value="{{ $filterYear ?? '' }}"
        ...
        <div class="space-y-3 pt-1 border-t border-border/70">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div>
                    <label class="block text-sm text-foreground mb-2">Jenis Jabatan ASN</label>
                    <x-ui.select
    ...
    <div class="px-4 sm:px-6 py-4 border-t border-border bg-gray-50/50 flex flex-col-reverse sm:flex-row items-center justify-between shrink-0 gap-3">
        ...
        <button type="button"
            ...
            hx-include="#retirement-filter-form, [name='search']"
            hx-target="#retirement-container" hx-select="#retirement-container" hx-swap="outerHTML" hx-push-url="true"
