{{-- File: resources/views/pages/admin/personnel/promotions/show/index.blade.php --}}
@extends('layouts.main.admin')

@section('title', 'Detail Kepangkatan: ' . $staff->name)
@section('page_title', 'Riwayat Kepangkatan Pegawai')
@section('page_subtitle', 'Detail riwayat kepangkatan untuk ' . $staff->name)
...
<div class="px-5 py-8 md:p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-secondary">Kelola riwayat kepangkatan untuk <span class="font-bold text-foreground">{{ $staff->name }}</span>.</p>
        </div>
        <div class="flex items-center gap-2 sm:gap-3 w-full md:w-auto mt-2 md:mt-0">
            <button type="button"
                hx-get="{{ route('admin.personnel.promotions.create', $staff->id) }}"
    ...
    {{-- 3. TABEL RIWAYAT KEPANGKATAN --}}
    <div class="bg-white rounded-2xl border border-border p-5">
        <div class="mb-5">
            <h2 class="text-lg font-bold text-foreground">Daftar Riwayat Kepangkatan (Golongan)</h2>
        </div>

        @include('pages.admin.personnel.grade.show.partials._table', compact('staff', 'histories'))
    </div>
</div>
@endsection