<div id="education-levels-container" class="animate-fade-in"
    hx-get="{{ request()->fullUrl() }}"
    hx-trigger="refreshEducationLevels from:body"
    hx-swap="outerHTML"
    hx-push-url="true"
    data-no-loader>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2.5">
                <h2 class="text-xl font-bold text-foreground">Data Tingkat Pendidikan</h2>
                <span class="inline-flex items-center justify-center min-w-[22px] h-[22px] px-1.5 rounded-full bg-primary/10 text-primary text-xs font-bold">
                    {{ $educationLevels->total() }}
                </span>
            </div>
            <p class="text-sm text-secondary mt-0.5">Daftar referensi tingkat pendidikan formal (SD, SMP, S1, dst).</p>
        </div>

        <button type="button"
            hx-get="{{ route('admin.master.education-levels.create') }}"
    ...
    <div class="hidden md:block overflow-x-auto">
        <table class="w-full text-left">
            <thead>
                <tr class="border-b border-border">
                    <th class="w-[45%] px-4 py-3 text-sm font-bold text-secondary tracking-wider">
                        Tingkat Pendidikan
                        <div class="text-[11px] font-normal normal-case">Nama | Level</div>
                    </th>
                    <th class="w-[45%] px-4 py-3 text-sm font-bold text-secondary tracking-wider">
                        Informasi
                        <div class="text-[11px] font-normal normal-case">Alias / Singkatan</div>
                    </th>
            ...
            <tbody>
                @forelse ($educationLevels as $item)
                @php $color = $badgeColors[$loop->index % count($badgeColors)]; @endphp
                <tr id="row-education-level-{{ $item->id }}" class="border-b border-border hover:bg-muted/50 transition-colors">
                    ...
                    <td class="px-5 py-4 min-w-[120px]">
                        <div x-data="{ open: false, menuX: 0, menuY: 0, toggle() { if(this.open) { this.open = false; return; } this.open = true; this.$nextTick(() => { const btn = this.$refs.button.getBoundingClientRect(); const menu = this.$refs.menu.getBoundingClientRect(); const spaceBelow = window.innerHeight - btn.bottom; const spaceAbove = btn.top; const dropUp = spaceBelow < menu.height && spaceAbove > menu.height; this.menuX = btn.right - menu.width; this.menuY = dropUp ? (btn.top - menu.height - 4) : (btn.bottom + 4); }); } }" @click.outside="open = false" @scroll.window="open = false" @resize.window="open = false" class="relative inline-block text-left">
                            ...
                            <div x-ref="menu" x-show="open" x-cloak x-transition class="fixed z-[9999] w-48 rounded-xl border border-border bg-white shadow-lg py-3 flex flex-col text-left" :style="`top: ${menuY}px; left: ${menuX}px;`">
                                ...
                                <button type="button" @click="open = false" hx-get="{{ route('admin.master.education-levels.edit', $item->id) }}" hx-target="#modal-container" hx-swap="innerHTML" class="flex items-center gap-2 mx-2 px-3 py-2 rounded-lg text-sm text-foreground hover:bg-muted transition-colors cursor-pointer text-left">
                                    <i data-lucide="file-pen-line" class="size-4 text-secondary"></i> Edit Data
                                </button>
                                <div class="my-2 border-t border-border"></div>
                                <button type="button" @click="open = false; ShowConfirm({ title: 'Hapus Tingkat Pendidikan?', message: 'Yakin ingin menghapus data ini?', confirmText: 'Ya, Hapus', cancelText: 'Batal' }, () => { htmx.ajax('DELETE', '{{ route('admin.master.education-levels.destroy', $item->id) }}', { swap: 'none', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content ?? '{{ csrf_token() }}' } }); })" class="flex items-center gap-2 mx-2 px-3 py-2 rounded-lg text-sm text-error hover:bg-error/10 transition-colors cursor-pointer text-left">
                ...
                <tr>
                    <td colspan="3" class="px-4 py-16 text-center text-secondary">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="font-medium">Belum ada data tingkat pendidikan.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="md:hidden divide-y divide-border border-y border-border mt-2 mb-4">
        ...
        <div class="py-3.5 flex items-center gap-3">
            <div class="size-9 rounded-xl {{ $color['bg'] }} {{ $color['text'] }} flex items-center justify-center font-bold text-xs shrink-0">{{ substr($item->alias, 0, 3) }}</div>
            <div class="min-w-0 flex-1">
                <div class="text-sm font-semibold text-foreground truncate">{{ $item->name }}</div>
                <div class="text-xs text-secondary mt-0.5 truncate">Alias: <span class="font-medium text-foreground">{{ $item->alias }}</span></div>
            </div>
            <div class="flex items-center gap-1.5 shrink-0">
                <button type="button" hx-get="{{ route('admin.master.education-levels.edit', $item->id) }}" hx-target="#modal-container" hx-swap="innerHTML" class="size-8 flex items-center justify-center rounded-lg border border-border bg-white text-secondary hover:text-blue-600 hover:border-blue-600 transition-colors">
                    <i data-lucide="file-pen-line" class="size-4"></i>
                </button>
                <button type="button" @click="ShowConfirm({ title: 'Hapus?', message: 'Yakin hapus data ini?', confirmText: 'Ya', cancelText: 'Batal' }, () => { htmx.ajax('DELETE', '{{ route('admin.master.education-levels.destroy', $item->id) }}', { swap: 'none', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }); })" class="size-8 flex items-center justify-center rounded-lg border border-border bg-white text-secondary hover:text-error hover:border-error transition-colors">
    ...
    <div class="mt-5">
        <x-ui.pagination :paginator="$educationLevels" hxTarget="#education-levels-container" />
    </div>
