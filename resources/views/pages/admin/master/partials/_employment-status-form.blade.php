@php
$isEdit = !empty($item);
$modalTitle = $isEdit ? 'Edit Status Kepegawaian' : 'Tambah Status Kepegawaian';
$actionUrl = $isEdit
? route('admin.master.employment-statuses.update', $item->id)
: route('admin.master.employment-statuses.store');
        ...
        <form id="employment-status-form"
            ...
            <div class="p-4 sm:p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $item->code ?? '') }}" required placeholder="Contoh: PNS" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Nama Status <span class="text-error">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $item->name ?? '') }}" required placeholder="Contoh: Pegawai Negeri Sipil" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
