@php
$isEdit = !empty($item);
$modalTitle = $isEdit ? 'Edit Tingkat Pendidikan' : 'Tambah Tingkat Pendidikan';
$actionUrl = $isEdit
? route('admin.master.education-levels.update', $item->id)
: route('admin.master.education-levels.store');
    ...
    <x-ui.modal show="open" maxWidth="md">
        ...
        <form id="education-level-form"
            ...
            <div class="p-4 sm:p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Nama Tingkat Pendidikan <span class="text-error">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $item->name ?? '') }}" required placeholder="Contoh: Strata 1" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1.5">Alias / Singkatan <span class="text-error">*</span></label>
                        <input type="text" name="alias" value="{{ old('alias', $item->alias ?? '') }}" required placeholder="Contoh: S1" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1.5">Level Urutan <span class="text-error">*</span></label>
                        <input type="number" name="level" value="{{ old('level', $item->level ?? '') }}" required placeholder="Contoh: 40" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        <p class="text-[11px] text-secondary mt-1">Angka untuk mengurutkan (SD=10, SMP=20, dst).</p>
                    </div>
                </div>
