@php
$isEdit = !empty($item);
$modalTitle = $isEdit ? 'Edit Golongan' : 'Tambah Golongan';
$actionUrl = $isEdit
? route('admin.master.grades.update', $item->id)
: route('admin.master.grades.store');
        ...
        <form id="grade-form"
            ...
            <div class="p-4 sm:p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1.5">Kode Golongan <span class="text-error">*</span></label>
                        <input type="text" name="grade_code" value="{{ old('grade_code', $item->grade_code ?? '') }}" required placeholder="Contoh: III/a" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1.5">Level/Tingkat <span class="text-error">*</span></label>
                        <input type="text" name="level" value="{{ old('level', $item->level ?? '') }}" required placeholder="Contoh: 31" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Nama Pangkat <span class="text-error">*</span></label>
                    <input type="text" name="grade_name" value="{{ old('grade_name', $item->grade_name ?? '') }}" required placeholder="Contoh: Penata Muda" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
            </div>
