@php
$isEdit = !empty($item);
$modalTitle = $isEdit ? 'Edit Jenis Personel' : 'Tambah Jenis Personel';
$actionUrl = $isEdit
? route('admin.master.personnel-types.update', $item->id)
: route('admin.master.personnel-types.store');
    ...
    <x-ui.modal show="open" maxWidth="md">
        ...
        <form id="personnel-type-form"
            ...
            <div class="p-4 sm:p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Kode <span class="text-error">*</span></label>
                    <input type="text" name="code" value="{{ old('code', $item->code ?? '') }}" required placeholder="Contoh: GTK" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Nama <span class="text-error">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $item->name ?? '') }}" required placeholder="Contoh: Guru dan Tenaga Kependidikan" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground mb-1.5">Alias</label>
                    <input type="text" name="alias" value="{{ old('alias', $item->alias ?? '') }}" placeholder="Singkatan (opsional)" class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
            </div>
